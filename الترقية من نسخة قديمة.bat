@echo off
chcp 65001 >nul
title ترقية نظام المحاسبة المتكامل من نسخة قديمة
echo ============================================================
echo   ترقية نظام المحاسبة المتكامل من نسخة قديمة إلى الجديدة
echo ============================================================
echo.

REM 1) إيقاف أي نسخة قديمة شغّالة على المنفذ 5000
echo [1/3] إيقاف أي نسخة شغّالة من النظام ...
powershell -NoProfile -WindowStyle Hidden -Command "Get-NetTCPConnection -LocalPort 5000 -State Listen -ErrorAction SilentlyContinue | ForEach-Object { Stop-Process -Id $_.OwningProcess -Force -ErrorAction SilentlyContinue }"
...
if exist "نظام المحاسبة المتكامل.exe" (
    "نظام المحاسبة المتكامل.exe" --upgrade
) else (
...
echo   الآن افتح النظام من ملف:  تشغيل السيستم.vbs
